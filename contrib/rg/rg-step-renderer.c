...
static void rg_step_renderer_init_renderer (RgRendererInterface *iface);

G_DEFINE_TYPE_WITH_CODE (RgStepRenderer, rg_step_renderer, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (RG_TYPE_RENDERER,
                                                rg_step_renderer_init_renderer))
...
RgStepRenderer *
rg_step_renderer_new (void)
{
  return g_object_new (RG_TYPE_STEP_RENDERER, NULL);
}
...
static void
rg_step_renderer_render (RgRenderer                  *renderer,
                         RgTable                     *table,
                         gint64                       x_begin,
                         gint64                       x_end,
                         gdouble                      y_begin,
...
{
  RgStepRenderer *self = (RgStepRenderer *)renderer;
  RgTableIter iter;

  g_assert (RG_IS_STEP_RENDERER (self));
...
static void
rg_step_renderer_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
  RgStepRenderer *self = RG_STEP_RENDERER (object);
...
static void
rg_step_renderer_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_STROKE_COLOR:
      rg_step_renderer_set_stroke_color (self, g_value_get_string (value));
      break;

    case PROP_STROKE_COLOR_RGBA:
      rg_step_renderer_set_stroke_color_rgba (self, g_value_get_boxed (value));
      break;
...
static void
rg_step_renderer_class_init (RgStepRendererClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->get_property = rg_step_renderer_get_property;
  object_class->set_property = rg_step_renderer_set_property;
...
static void
rg_step_renderer_init (RgStepRenderer *self)
{
  self->line_width = 1.0;
}

static void
rg_step_renderer_init_renderer (RgRendererInterface *iface)
{
  iface->render = rg_step_renderer_render;
}

void
rg_step_renderer_set_stroke_color_rgba (RgStepRenderer *self,
                                        const GdkRGBA  *rgba)
{
  const GdkRGBA black = { 0, 0, 0, 1.0 };

  g_return_if_fail (RG_IS_STEP_RENDERER (self));
...
void
rg_step_renderer_set_stroke_color (RgStepRenderer *self,
                                   const gchar    *stroke_color)
{
  ...
  if (gdk_rgba_parse (&rgba, stroke_color))
    rg_step_renderer_set_stroke_color_rgba (self, &rgba);
}
